...
void createLocalMutation(int num_ranks, int rank, double mutationRatio){
  ...
  for(i = 0; i< count_mutants; i++)
	...
	if (index1!=index2){
      ...
      individual = rand()%(popSize/num_ranks);
...
/*
void syncMutationData(int num_ranks, int rank){
  //First allgather count_mutants
  // Then allgatherv global mutation data
  ...
  int *sendbuf;
  sendbuf = &count_mutants;
  int sendcount = 1;
  int *recvbuf = &mutation_counts_at_ranks[0];
  int recvcount = 1;
  ...
  for(i=1;i<num_ranks;i++){
    displacements[i] = displacements[i-1]+dataLengths[i-1];
  }
  ...
  if (1==rank)
    {
      for(i=0;i<num_ranks; i++){
	printf("Displacements %d,   %d\n",rank,displacements[i]);
	
      }
      for(i=0;i<num_ranks; i++){
	printf("datalengths %d,   %d\n",rank,dataLengths[i]);
  ...
  globalMutationData = calloc(displacements[num_ranks-1], sizeof(int));
  
  MPI_Allgatherv(localMutationData, count_mutants*3, MPI_INT,
  ...
  if(1==rank){
	...
	printf("From sync func  rank %d,   ind %d   ind1 %d    ind2 %d \n",
	       i,
	       globalMutationData[ displacements[i] + j*3 ],
	       globalMutationData[ displacements[i] + j*3 + 1],
	       globalMutationData[ displacements[i] + j*3 + 2]
	       );
      }
