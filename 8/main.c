...
void findMin(int popSize,int num_ranks,int rank, double *fitness);

void syncMins(int num_ranks, int rank);

void findGlobalMin(int num_ranks, int rank);
...
// Function to synchronize population across ranks
  ...
  minAtRank = calloc(num_ranks,sizeof(double));
  minIndexAtRank = calloc(num_ranks,sizeof(int));
...
// Calculate Fitness Values
  ...
  findMin(popSize, num_ranks, rank, fitness);
  // Synchronizing the minimums at each rank and their indices
  syncMins(num_ranks,rank);
  ...
  findGlobalMin(num_ranks, rank);

  if(0==rank){
    printf("The global minimum is %f  at index %d \n",globalMin, globalMinIndex);
  }
...
void findMin(int popSize, int num_ranks, int rank, double *fitness){
  ...
  int i;
  double currentmin=fitness[0];
  int minindex = 0;
  for (i=0; i< popSize/num_ranks; i++){
    if (currentmin > fitness[i]){
      currentmin = fitness[i];
      minindex = i;
    }
  }
  minAtRank[rank] = currentmin;
  minIndexAtRank[rank] = minindex + rank*popSize/num_ranks;
...
void syncMins(int num_ranks, int rank){
  ...
  MPI_Allgather(&minAtRank[rank], 1, MPI_DOUBLE,
		minAtRank, 1, MPI_DOUBLE, MPI_COMM_WORLD);
  
  MPI_Allgather(&minIndexAtRank[rank], 1, MPI_INT,
		minIndexAtRank, 1, MPI_INT, MPI_COMM_WORLD);
...
void findGlobalMin(int num_ranks, int rank){
  ...
  int i;
  int min = minAtRank[0];
  int minindex = minIndexAtRank[0];
  
  for(i=0; i<num_ranks; i++){
    if (min > minAtRank[i]){
      min = minAtRank[i];
      minindex = minIndexAtRank[i];
    }
  }
